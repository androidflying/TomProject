...
import com.tom.kalle.cookie.db.CookieDao;
import com.tom.kalle.cookie.db.Field;
import com.tom.kalle.cookie.db.Where;
...
public class DBCookieStore implements CookieStore, Field {
        ...
        try {
            ...
            for (Cookie cookie : cookieList) {
                if (!Cookie.isExpired(cookie)) {
                    returnedCookies.add(Cookie.toHttpCookie(cookie));
                }
            }
    ...
    public void remove(HttpCookie httpCookie) {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(domain)) {
                whereBuilder.and(DOMAIN, Where.Options.EQUAL, domain);
            }
