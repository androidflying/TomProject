...
import com.tom.kalle.body.ByteArrayBody;
import com.tom.kalle.cache.Cache;
import com.tom.kalle.cache.CacheMode;
import com.tom.kalle.cache.CacheStore;
import com.tom.kalle.converter.Converter;
import com.tom.kalle.exception.NoCacheError;
import com.tom.kalle.exception.ParseError;
...
import static com.tom.kalle.http.Headers.KEY_IF_MODIFIED_SINCE;
import static com.tom.kalle.http.Headers.KEY_IF_NONE_MATCH;
...
abstract class BasicWorker<T extends SimpleRequest, Succeed, Failed> implements Callable<SimpleResponse<Succeed, Failed>> {
    ...
    public final SimpleResponse<Succeed, Failed> call() throws Exception {
        ...
        if (response != null) {
            return buildSimpleResponse(response, true);
        }
    ...
    private void tryAttachCache() {
        ...
        switch (cacheMode) {
            ...
            case HTTP_YES_THEN_WRITE_CACHE: {
                ...
                if (cacheEntity != null) {
                    attachCache(cacheEntity.getHeaders());
                }
                break;
            }
            case NETWORK:
    ...
    private void attachCache(Headers cacheHeaders) {
        ...
        if (eTag != null) {
            headers.set(KEY_IF_NONE_MATCH, eTag);
        }
